      ...
      <div className="max-w-6xl mx-auto">
        <ClientWrapper>
            ...
            <section className="mb-8">
              ...
              <div className="grid gap-3">
                {activeClients.map((client: any) => (
                  <div key={client.id} className="bg-slate-800 rounded-xl p-4 border border-slate-700 hover:border-blue-500/50 transition-colors">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
                      <div>
                        <div className="font-semibold">{client.name}</div>
                        {client.domain && (
                          <a href={`https://${client.domain}`} target="_blank" className="text-sm text-slate-400 hover:text-blue-400">
                            {client.domain} ↗
                          </a>
                        )}
                      </div>
                  ...
                  <div key={client.id} className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-2">
                      <div>
                        ...
                        {client.domain && (
                          <span className="text-sm text-slate-500">{client.domain}</span>
                        )}
                      </div>
                      {client.potential_budget && (
                        <div className="text-amber-400/70 font-mono text-sm">
                          ~{client.potential_budget.toLocaleString('ru-RU')} ₽
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {recommendations.slice(0, 5).map((rec: any) => (
                  <div key={rec.id} className="bg-slate-800 rounded-xl p-4 border border-slate-700">
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-purple-400">📈 +{rec.potential_impressions?.toLocaleString('ru-RU')} показов</span>
                      {rec.priority && (
          ...
          <footer className="mt-8 pt-6 border-t border-slate-800 text-center text-slate-500 text-sm">
            ...
            <p className="mt-1">
              <a href="https://artvision.pro" className="hover:text-blue-400">artvision.pro</a>
            </p>
          </footer>
        </ClientWrapper>
      </div>
